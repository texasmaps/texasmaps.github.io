...
WM.siteList=function(){if(!cfg.listMetric)return '';
  const chips=[['operating','Operating'],['under_construction','Under construction'],['proposed','Proposed']].map(([k,l])=>`<button type="button" class="chip${fstate.status.has(k)?'':' off'}" data-s="${k}"><i class="st-${k}" style="background:${COL[k]}"></i>${l}</button>`).join('')+`<button type="button" class="chip${fstate.hot?' on':''}" data-hot="1"><i></i>Contested only</button>`;
...
WM.init=function(c){
  ...
  SITES.forEach(s=>{const [x,y]=px(s.lon,s.lat);const c=svgEl('circle');c.setAttribute('cx',x);c.setAttribute('cy',y);c.setAttribute('r',rad(s));c.setAttribute('fill',COL[s.status]||'#888');c.setAttribute('class','pin st-'+s.status+(s.hot?' hot':''));c.dataset.id=s.id;s.el=c;gP.appendChild(c);});
...
function initWelcome(){let seen=false;try{seen=localStorage.getItem('txwater_welcome')==='1';}catch(e){}if(seen||cfg.noWelcome)return;
  const tips=cfg.welcomeTips||['Every dot is a data center. Bigger dots are bigger projects, and a red ring means local opposition.','Hover a dot or a county for details; click for the full water story.','Use <b>Map overlays</b> (top right) to color the map by aquifers, rainfall or wells.'];
...
function initAbout(){const here=(location.pathname.split('/').pop()||'');const panel=document.createElement('div');panel.className='about';panel.innerHTML=`<div class="aboutbox"><button class="close" aria-label="Close">×</button><h2>About this map</h2>${cfg.about||''}
    <h3>Where the data comes from</h3><ul><li><b>Data center sites</b>: Texas Data Center Watch, which tracks announced projects across the state. A dot sits at the reported site when it is known, otherwise at the city or county center; each site card says which.</li>
    <li><b>Aquifers, rivers, lakes, river basins and well records</b>: the Texas Water Development Board, the state's water agency. TWDB updates its well records nightly; these maps use a copy from ${esc((WM.cw&&WM.cw.generated)||'the date shown on the wells map')}.</li>
    ...
    <h3>Rivers, river basins and lakes</h3><p><b>Rivers</b> are the channels themselves: the 21 major rivers such as the Rio Grande, Brazos, Colorado and Trinity. <b>River basins</b> are the land that drains to each river; every place in Texas is in exactly one, their boundaries follow ridgelines rather than water, and they are the units the state uses for water planning and water rights. <b>Lakes</b> on these maps are reservoirs, the man-made lakes behind dams that store river water; 188 of the 211 shown are classed as water supply.</p>
...
function legend(){const L=$('legend');if(!L)return;let h='';
  if(state.pins)h+='<b>Data center sites</b>'+['operating','under_construction','proposed'].map(k=>`<div class="r"><i class="st-${k}" style="background:${COL[k]}"></i>${LBL[k]}</div>`).join('')+'<div class="r"><i class="ring"></i>Red ring: local opposition or a lawsuit</div><div class="foot">Bigger dots are bigger projects (announced power).</div>';
...
WM.siteRow=function(s,val){return `<div class="row" data-site="${esc(s.id)}" tabindex="0"><span class="sw dot st-${esc(s.status)}${s.hot?' hot':''}" style="background:${COL[s.status]}"></span><div><div class="n">${esc(s.name)}</div><div class="m">${esc(s.operator||'')}${s.operator?' · ':''}${esc(s.city||'')}, ${esc(s.county)}</div></div><span class="v">${val??(s.mw?fmtMW(s.mw):'')}</span></div>`;};
